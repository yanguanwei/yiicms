...
class AdminForm extends CWidget
{
  ...
  public function renderChannelSelectField($attribute, $parent_id, array $htmlOptions = array())
  {
    ...
    $command = $conn->createCommand(
      "SELECT id, title FROM {{channel}} WHERE parent_id='{$parent_id}' AND `visible`='1' AND `model`='' ORDER BY sort_id DESC, id ASC"
    );
  ...
  public function renderTreeSelectField(
    $attribute,
    array $data,
    $rootId = 0,
    $note = null,
  ...
  public function treeSelect($attribute, array $data, $rootId = 0, array $htmlOptions = array())
  {
    ...
    return $this->widget(
      'application.widgets.UnlimitedSelect',
      array(
        'model' => $model,
        'attribute' => $attribute,
        'data' => $data,
  ...
  public function renderTagSelectField(array $tagTypeNames, $array = null)
  {
    $selects = array();
    ...
    foreach ($tagTypes as $name => $typeTitle) {
      ...
      $attribute = $array === null ? $name : "{$array}[$name]";
      $selects[] = $this->form->dropDownList($this->model, $attribute, $options, array('empty' => '--' . $tagTypes[$name] . '--'));
    }
  ...
  {
    if (isset($this->model->cid)) {
      $tags = Channel::getChannelTags($this->model->cid);
      return $this->renderTagSelectField($tags, 'tags');
    }
  }
  ...
  protected function parseField($config)
  {
    if (is_array($config)) {
      ...
      if (isset($config['note']) && $config['note']) {
        $note = "<br /><small>{$config['note']}</small>";
      }
    } else {
      $attribute = $config;
      $type = 'textField';
