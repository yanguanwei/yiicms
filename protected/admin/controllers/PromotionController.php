...
class PromotionController extends AdminController
{
	...
	{
		$form = new PromotionForm('insert');
		
		if ( isset($_POST['PromotionForm']) ) {
			
			if ( $form->post($_POST['PromotionForm'], true) ) {
	...
	{
		$form = new PromotionForm('update');
		
		if ( isset($_POST['PromotionForm']) ) {
			
			if ( $form->post($_POST['PromotionForm'], false) ) {
			...
			$news = Promotion::model()->with('archive')->findByPk($id);
			if ( !$news ) {
	...
	public function actionDelete()
	{
		...
		$count = Promotion::deletePromotion($id);
