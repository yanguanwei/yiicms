...
class Channel extends CActiveRecord
{
	...
	public static function getChildrenIds($parent_id)
	{
		...
		while (null != $data = $command->select('id')->from('{{channel}}')->where(array('and', array('in', 'parent_id', $parent_id), "visible='1'"))->queryAll()) {
			$parent_id = array();
			foreach ($data as $row) {
	...
	public static function getTreeTrace($id, $attribute)
	{
		...
		while (null != $row = $command->select("id, parent_id, {$attribute}")->from('{{channel}}')->where('id=:id', array(':id' => $id))->queryRow()) {
			$id = $row['parent_id'];
	...
	{
		$rootId = 0;
		while (false !== $row = Yii::app()->db->createCommand("SELECT parent_id FROM {{channel}} WHERE id='{$id}'")->queryRow()) {
			if ($row['parent_id']) {
				$id = $row['parent_id'];
			} else {
				$rootId = intval($id);
	 ...
	 * @return number
	 */
	public static function countFromModel($id)
	{
		$model = self::getChannelModel($id);
		if ( $model ) {
			$sql = "SELECT count(*) as model_count FROM {{{$model}}} WHERE cid='{$id}'";
			$row = Yii::app()->db->createCommand($sql)->queryRow();
			if ( $row )
				return intval($row['model_count']);
		}
		return 0;
	}
	 ...
	 * @return string|null
	 */
	public static function getChannelModel($id)
	{
	...
	public static function getChannelSelectOptions($theme_id, $model_id)
	{
		...
		$conn = Yii::app()->db;
		$command = $conn->createCommand("SELECT id, title FROM {{channel}} WHERE theme_id='{$theme_id}' AND `model_id`='{$model_id}' AND `visible`='1' ORDER BY sort_id DESC, id ASC");
